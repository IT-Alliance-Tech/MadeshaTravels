...
export default function BlogSection() {
  ...
  const handlePrev = () => setIndex((prev) => (prev === 0 ? blogs.length - 1 : prev - 1));
  const handleNext = () => setIndex((prev) => (prev === blogs.length - 1 ? 0 : prev + 1));
  ...
  return (
    <section className={styles.blogSection}>
      ...
      <div className={styles.navButtons}>
        ...
        <button className={`${styles.navBtn} ${styles.activeBtn}`} onClick={handleNext}>→</button>
      </div>
    </section>
  );
}
